...
function BarChart({ data, geselecteerdeWaarde }) {
	...
	const ref = useD3(
		svg => {
			...
			const onMouseMove = (d, data) => {
				...
				toolTipValue = data[geselecteerdeWaarde]
				...
				d3.select('#naam').text(`${data.name} heeft `)

				// Ik gebruik hier een if else statement, omdat ik voor iedere filter een andere text wil laten zien
				if (geselecteerdeWaarde === 'playcount') {
					d3.select('#waarde').text(`${toolTipValue} totaal aantal streams `)
				}
				if (geselecteerdeWaarde === 'listeners') {
					d3.select('#waarde').text(`${toolTipValue} aantal verschillende luisteraars `)
				}
				if (geselecteerdeWaarde === 'average') {
					d3.select('#waarde').text(
			...
			const vierkant = () => {
				...
				rect.transition()
					.duration(1000)
					.attr('width', d => xscale(d[geselecteerdeWaarde]))
			...
			const sorteren = data => {
				data.sort((a, b) => {
					return b[geselecteerdeWaarde] - a[geselecteerdeWaarde] // b - a omdat ik hoogste waarde bovenaan wil zetten. Als ik de laagste waarde bovenaan wil zetten wordt het a - b
				})
			}

			// Hier geef ik data mee aan de X en de Y as
			const assenUpdate = data => {
				xscale.domain([0, d3.max(data, d => +d[geselecteerdeWaarde])])
			...
			update(data, geselecteerdeWaarde)
		},
		[data.length, geselecteerdeWaarde]
	)
